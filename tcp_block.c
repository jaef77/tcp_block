...
int tcp_block(const uint8_t* p, int len);
int send_rst_fin(pcap_t *handle, eth_hdr_custom *ETH, ip_hdr_custom *IP, tcp_hdr_custom *TCP, uint8_t *my_mac, int packet_len, int what_flag, int if_forward);
...
int main(int argc, char *argv[])
{
    ...
    while (true) {
        ...
        if(tcp_block(packet, header->caplen))
        {
...
int send_rst_fin(pcap_t *handle, eth_hdr_custom *ETH, ip_hdr_custom *IP, tcp_hdr_custom *TCP, uint8_t *my_mac, int packet_len, int what_flag, int if_forward)
{
    ...
    eth_hdr_custom *send_ETH;
    ip_hdr_custom *send_IP;
    tcp_hdr_custom *send_TCP;
    uint8_t *send_TCP_DATA;

    const uint32_t eth_header_length = 14;
    uint16_t ip_header_length = 4 * IP->ip_hl;
    uint16_t tcp_header_length = 4 * TCP->tcp_hlen;
    uint16_t tcp_data_length = packet_len - tcp_header_length - ip_header_length - eth_header_length;

    // Copy the packet
    memcpy(send_ETH, ETH, eth_header_length);
    memcpy(send_IP, IP, ip_header_length);
    memcpy(send_TCP, TCP, tcp_header_length);
    memcpy(send_TCP_DATA, (uint8_t *)(TCP + tcp_header_length), tcp_data_length);

    /********************************************************************************/
    // Check if FORWARD or BACKWARD ==> change the source/destination/tcp seq,ack
    memcpy(send_ETH->source_mac, my_mac, sizeof(eth_hdr_custom->source_mac));

    if(if_forward == TCP_BACKWARD)
    {
        memcpy(send_ETH->dest_mac, ETH->souce_mac, sizeof(eth_hdr_custom->dest_mac));
        send_IP->ip->ttl = 0;
    ...
    if(what_flag == TCP_FIN + TCP_ACK)      // FIN
        tcp_length = tcp_hdr_length + tcp_data_length;
    else                                    // RST
    {
        tcp_length = tcp_hdr_length;
        tcp_data_length = 0;
    }

    send_IP->ip_len = htons(ip_hdr_length + tcp_length);
    ...
    if(tcp_data_length % 2)
    {
        tcp_check += ntohs(tcp_data[tcp_data_length / 2])
    }
    ...
    send_TCP->tcp_check = htons(tcp_check) ^ 0xFFFF;
	...
	if(ntohs(ETH->eth_type) == ETHERTYPE_IP)
	{
		...
		if(IP->ip_pro == IPPROTO_TCP)
		{
            ...
            if(tcp_data_length > 0)
            {
                ...
                {
                    // Forward RST
                    send_rst_fin(handle, ETH, IP, TCP, my_mac, len, TCP_RST, TCP_FORWARD);

                    // Backward FIN
                    send_rst_fin(handle, ETH, IP, TCP, my_mac, len, TCP_FIN + TCP_ACK, TCP_BACKWARD);
                }
                ...
                else    // tcp
                {
                    ...
                    // Backward RST
                    send_rst_fin(handle, ETH, IP, TCP, my_mac, len, TCP_RST, TCP_BACKWARD);
                }
            }
		}
        else
